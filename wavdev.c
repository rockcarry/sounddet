...
static BOOL CALLBACK waveInProc(HWAVEIN hWav, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
    ...
    switch (uMsg) {
    case WIM_DATA:
        if (dev->incallback) dev->incallback(dev, dev->incbctxt, phdr->lpData, phdr->dwBytesRecorded);
...
static void CALLBACK waveOutProc(HWAVEOUT hwo, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
    ...
    switch (uMsg) {
    case WOM_DONE:
        if (dev->outcallback) dev->outcallback(dev, dev->outcbctxt, phdr->lpData, phdr->dwBytesRecorded);
